...
# Copyright (C) 2014-2016 Blackosx
# 
...
if [ $osVer -ge 14 ]; then # Yosemite and newer
    ...
    gSipStr="${gCSR_ALLOW_DEVICE_CONFIGURATION},${gCSR_ALLOW_UNRESTRICTED_NVRAM},${gCSR_ALLOW_UNRESTRICTED_DTRACE},${gCSR_ALLOW_APPLE_INTERNAL},${gCSR_ALLOW_KERNEL_DEBUGGER},${gCSR_ALLOW_TASK_FOR_PID},${gCSR_ALLOW_UNRESTRICTED_FS},${gCSR_ALLOW_UNTRUSTED_KEXTS}"
    export gSipStr

    gLkStr="${gDrvLoadedD},${gDrvLoadedP},${gDrvLoadedR},${gDrvLoadedV}"
    export gLkStr
    ...
    if [ $# -ne 0 ]; then
            ...
            case $opt in
              ...
              p) # PRECONFIGURED - Check the arguments
                 if [ "$OPTARG" == "1" ]; then
                     ...
                     argumentsToPass=",ArchiveZip,acpi,cpuinfo,devprop,dmi,edid,bootlogF,bootlogK,ioreg,kernelinfo,rcscripts,nvram,opencl,power,rtc,sip,smc,sysprof,Report"
                     wasDumpChosen=1
                 elif [ "$OPTARG" == "2" ]; then
                    # Run as root
                     argumentsToPass=",ArchiveZip,acpi,acpiFromMem,codecid,biosSystem,biosVideo,cpuinfo,diskLoaderConfigs,devprop,bootLoaderBootSectors,diskVolumeXuid,diskPartitionInfo,dmi,edid,bootlogF,bootlogK,firmmemmap,memory,ioreg,kernelinfo,kexts,lspci,rcscripts,nvram,opencl,power,rtc,sip,smc,sysprof,Report"
                     wasDumpChosen=1
                 elif [ "$OPTARG" == "3" ]; then
                     # Run all without root privileges and with privacy enabled
                     argumentsToPass=",ArchiveZip,acpi,cpuinfo,devprop,dmi,edid,bootlogF,bootlogK,ioreg,kernelinfo,rcscripts,nvram,opencl,rtc,sip,smc,sysprof,Report,privacy"
                     wasDumpChosen=1
                 elif [ "$OPTARG" == "4" ]; then
        ...
        if [ ! "$argumentsToPass" == "" ]; then
            ...
            if [ $wasDumpChosen -eq 1 ]; then
                ...
                declare -a rootDumps=(acpiFromMem codecid biosSystem biosVideo diskLoaderConfigs bootLoaderBootSectors diskVolumeXuid diskPartitionInfo firmmemmap memory kexts lspci)
                rootFlag=0
